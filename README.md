### 👋 Hi, I'm Yasaswini Kalavakuri

Final-year BSc Computer Science student at the University of Southampton, interested in AI/ML, cloud systems, and building software that's actually useful.

- 🎓 Currently working on my dissertation: *Data Augmentation — An Extensive Study of Compression and Diversity*
- 🧩 This year's modules: Social Computing Techniques, Advanced Databases, Individual Dissertation Project
- 🌱 Learning: Deep Learning, Cloud Computing (Azure & AWS), Web Systems Architecture
- 💬 Ask me about: Python, Java, ML pipelines, or cloud-hosted apps
- 📫 Reach me: yasaswinikalavakuri@gmail.com

---

### ⚙️ Tech Stack

**Languages:** Python · Java · Haskell · JavaScript · SQL · C
**Frontend:** React · Tailwind CSS · HTML/CSS
**Backend:** Node.js · Express · Flask · JavaFX
**Databases:** PostgreSQL · MongoDB · CosmosDB
**Cloud & DevOps:** Azure Functions · AWS · Docker · CI/CD
**ML/AI:** PyTorch · Scikit-learn · Transformers · RAG pipelines

---

### 🧩 Featured Projects

| Project | Description | Tech |
|---|---|---|
| ✈️ [Runway Redeclaration Tool](https://github.com/Yashuk97/Runway-redeclaration-tool) | JavaFX tool automating runway safety calculations per CAA guidelines. Best Project recognition among coursework. | Java, JavaFX, MVC |
| 🎮 [TetrECS](https://github.com/Yashuk97/TETRECS-project) | Feature-complete Tetris-style game with custom scoring, multiplayer, and animated UI. | Java, JavaFX |
| ⚙️ [PLC_COMP2212](https://github.com/Yashuk97/PLC_COMP2212) | Domain-specific language (CQL) and interpreter for querying/transforming CSVs. | Haskell, Alex, Happy |
| 🗄️ [COMP1204 – Data Management](https://github.com/Yashuk97/COMP-1204---Data-Management-) | Relational modelling, normalisation (1NF–BCNF), and SQL analysis of a COVID-19 dataset. | SQL, SQLite |
| 🌐 [Maanasa Website](https://github.com/Yashuk97/Maanasa-Website) | Responsive business website for a construction company. | HTML, CSS, JS |

---

### 🏆 Achievements & Certifications

- 🥇 Best Project recognition — Runway Redeclaration Tool
- 🎓 RAG Certificate — Retrieval-Augmented Generation (Coursera)
- 💡 AI & ML Hackathon participant
- 💼 Internship experience in real-world software development

---

### 💬 Let's Connect

[LinkedIn](https://linkedin.com/in/yasaswini-kalavakuri) · [Portfolio](https://yashuk97.github.io) · [Email](mailto:yasaswinikalavakuri@gmail.com)
